...
describe("tab switching", () => {
  ...
  it("clicking Text tab activates it and hides Colors", async () => {
    ...
    expect(
      q('.pk-tab[data-panel="text"]').classList.contains("pk-active"),
    ).toBe(true);
    expect(
      q('.pk-tab[data-panel="colors"]').classList.contains("pk-active"),
    ).toBe(false);
...
describe("Save persists to storage but does not touch the real page", () => {
  ...
  it("Save alone does not push a new edit to document.documentElement (only Close does)", async () => {
    ...
    const baseline =
      document.documentElement.style.getPropertyValue("--pk-background");
