...
describe('applyText', () => {
  ...
  it('removes the font-family variable when fontFamily is "site"', () => {
    const target = createFakeTarget();
    target.style.setProperty('--pk-font-family', "'Atkinson Hyperlegible Next', sans-serif");
    applyText(
      {
        fontSize: 100,
        lineHeight: 1.5,
  ...
  it('sets the Atkinson Hyperlegible Next stack when fontFamily is "atkinson"', () => {
    ...
    expect(target.props['--pk-font-family']).toContain('Atkinson Hyperlegible Next');
  });
});
...
describe('applyFocus', () => {
  it('writes outline color as hsl() and width in px', () => {
    const target = createFakeTarget();
    applyFocus({ outlineColor: { hue: 200, sat: 100, light: 50 }, outlineWidth: 3 }, target);
    expect(target.props['--pk-focus-outline-color']).toBe('hsl(200, 100%, 50%)');
